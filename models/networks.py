...
def convTranspose_norm_relu_module(
    norm_type,
    norm_layer,
    input_nc,
    ngf,
...
def define_G_3d(
    input_nc, output_nc, norm="batch", groups=26, ksize=3, padding=1, gpu_ids=[]
):
    ...
    netG_3d = ResnetGenerator_3d_conv(
        input_nc,
        output_nc,
        norm_type=norm,
        groups=groups,
...
def define_preNet(
    input_nc, nif=32, which_model_preNet="none", norm="batch", gpu_ids=[]
):
    preNet = None
    norm_layer = get_norm_layer(norm_type=norm)
    use_gpu = len(gpu_ids) > 0
...
class ResnetGenerator_3d_conv(nn.Module):
    def __init__(
        ...
        model = [
            nn.Conv3d(
                input_nc, output_nc, kernel_size=ksize, padding=padding, groups=groups
            ),
...
class InputTransformation(nn.Module):
    def __init__(self, input_nc, nif=32, norm_layer=nn.BatchNorm2d, gpu_ids=[]):
        ...
        sequence = [
            nn.Conv2d(input_nc, nif, kernel_size=3, stride=2, padding=1),
            norm_layer(nif),
            nn.ReLU(True),
        ]
        sequence += [
...
class NLayerDiscriminator(nn.Module):
    def __init__(
        ...
        for n in range(1, n_layers):
            ...
            nf_mult = min(2 ** n, 8)
            sequence += conv_norm_relu_module(
                norm_type,
                norm_layer,
                ndf * nf_mult_prev,
                ndf * nf_mult,
