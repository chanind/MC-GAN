...
class StackGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # define tensors
        self.input_A0 = self.Tensor(
            opt.batchSize, opt.input_nc, opt.fineSize, opt.fineSize
        )
        self.input_B0 = self.Tensor(
            opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize
        )

        self.input_base = self.Tensor(
            opt.batchSize, opt.output_nc, opt.fineSize, opt.fineSize
        )
        ...
        if self.opt.conv3d:
            ...
            self.netG_3d = networks.define_G_3d(
                opt.input_nc,
                opt.input_nc,
                norm=opt.norm,
                groups=opt.grps,
        ...
        self.netE1 = networks.define_Enc(
            opt.input_nc_1,
            opt.output_nc_1,
            opt.ngf,
            opt.which_model_netG,
        ...
        self.netDE1 = networks.define_Dec(
            opt.input_nc_1,
            opt.output_nc_1,
            opt.ngf,
            opt.which_model_netG,
        ...
        if not self.isTrain:
            ...
            if opt.no_Style2Glyph:
                ...
                self.load_network(
                    self.netG, "G", str(int(opt.which_epoch) + int(opt.which_epoch1))
                )
        ...
        if self.isTrain:
            ...
            if self.opt.conv3d:
                self.optimizer_G_3d = torch.optim.Adam(
                    self.netG_3d.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
                )
            ...
            self.optimizer_E1 = torch.optim.Adam(
                self.netE1.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
            if opt.which_model_preNet != "none":
                self.optimizer_preA = torch.optim.Adam(
                    self.preNet_A.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
                )

            self.optimizer_DE1 = torch.optim.Adam(
                self.netDE1.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )

            self.optimizer_D1 = torch.optim.Adam(
                self.netD1.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999)
            )
    ...
    def all2observed(self, tensor_all):
        ...
        for batch in range(b):
            if not self.opt.rgb_in and self.opt.rgb_out:
                tensor_gt[batch, 0, :, :] = tensor_all.data[
                    batch, self.out_id[batch], :, :
                ]
                tensor_gt[batch, 1, :, :] = tensor_all.data[
                    batch, self.out_id[batch], :, :
                ]
                tensor_gt[batch, 2, :, :] = tensor_all.data[
                    batch, self.out_id[batch], :, :
                ]
            else:
                # TODO
    ...
    def forward1(self, inp_grad=False):
        ...
        obs_ = (
            torch.cuda.LongTensor(self.obs)
            if self.opt.gpu_ids
            else LongTensor(self.obs)
        )
    ...
    def backward_G(self, pass_grad, iter):
        ...
        if not self.opt.lambda_C or (iter > 700):
            ...
            weights = (
                torch.ones(b, c, m, n).cuda()
                if self.opt.gpu_ids
                else torch.ones(b, c, m, n)
            )
            obs_ = (
                torch.cuda.LongTensor(self.obs)
                if self.opt.gpu_ids
            ...
            self.loss_G_L1 = (
                self.criterionL1(
                    weights * self.fake_B0, weights * self.fake_B0_init.detach()
                )
                * self.opt.lambda_C
            )
    ...
    def backward_G1(self, iter):
        ...
        self.loss_G1_L1 = (
            self.criterionL1(self.fake_B1_gt, self.real_B1) * self.opt.lambda_A
        )
        ...
        self.loss_G1_MSE_rgb2gay = (
            self.criterionMSE(fake_B1_gray, real_A1_gray.detach())
            * self.opt.lambda_A
            / 3.0
        )
        ...
        self.loss_G1_MSE_gt = (
            self.criterionMSE(real_A1_gt_gray, real_B1_gray) * self.opt.lambda_A
        )
        ...
        self.real_A1_grad = (
            torch.zeros(b, c, m, n).cuda()
            if self.opt.gpu_ids
            else torch.zeros(b, c, m, n)
        )

        if G1_L1_update:
            for batch in self.obs:
                self.real_A1_grad[batch, :, :, :] = self.real_A1_gt_s.grad.data[
                    self.id_[batch], :, :, :
                ]
    ...
    def get_current_visuals(self):
        ...
        if self.opt.isTrain:
            ...
            real_A_all = self.Tensor(
                real_B.size(0),
                real_B.size(1),
                real_A1.size(2),
                real_A1.size(2) * real_A1.size(0),
            )
            fake_B_all = self.Tensor(
                real_B.size(0),
                real_B.size(1),
                real_A1.size(2),
            ...
            for b in range(g):
                real_A_all[
                    :, :, :, self.out_id[b] * m : m * (self.out_id[b] + 1)
                ] = real_A1[b, :, :, :]
                fake_B_all[
                    :, :, :, self.out_id[b] * m : m * (self.out_id[b] + 1)
                ] = fake_B[b, :, :, :]
