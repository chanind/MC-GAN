...
class FlatData(object):
    ...
    def __next__(self):
        ...
        if self.blanks != 0:
            ...
            if not self.dict:
                blank_ind = np.random.permutation(A.size(3) / target_size)[
            ...
            blank_ind = np.tile(range(target_size), len(blank_ind)) + np.repeat(
...
class Data(object):
    def __init__(
        self, data_loader, fineSize, max_dataset_size, rgb, dict_test={}, blanks=0.7
    ):
        self.data_loader = data_loader
        self.fineSize = fineSize
        self.max_dataset_size = max_dataset_size
    ...
    def __next__(self):
        ...
        A = AB[
            :,
            :,
            h_offset : h_offset + self.fineSize,
...
class PartialData(object):
    ...
    def __next__(self):
        ...
        for index in range(A.size(0)):
            ...
            if self.blanks != 0:
                gt_glyph = [
                    int(A_paths[index].split("/")[-1].split(".png")[0].split("_")[-1])
                ]
                observed_glyph = list(
...
class StackDataLoader(BaseDataLoader):
    ...
    def initialize(self, opt):
        ...
        dataset_B = ImageFolder(
            root=opt.dataroot + "B/" + opt.phase,
            transform=transform,
            return_paths=True,
            rgb=opt.rgb_out,
            fineSize=opt.fineSize,
