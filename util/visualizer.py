...
class Visualizer:
    ...
    def copy_images(
        self, image_paths, image_path, dest_path_A, dest_path_B, target_path, observed
    ):
        ...
        for im in target_ims:
            ...
            image_name = "%s_%s.png" % (name, obs)
            shutil.copyfile(
                os.path.join(target_path, im), os.path.join(dest_path_B, image_name)
            )
            ...
            for img_dir in self.video_dir:
                ...
                for im in imgs:
                    if im.endswith("_%s.png" % end_):
                        ...
                        os.system(
                            "cp %s ~/tmp_ffmpeg/%s/im%04d.png 2>&1|tee ~/tmp_ffmpeg/log.txt"
                            % (os.path.join(img_dir.replace("&", "\&"), im), end_, i)
                        )
                        i += 1
        os.system(
            "ffmpeg -r 6 -f image2 -s %dx%d -i ~/tmp_ffmpeg/fake_B/im%%04d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p %s/test_fake_B.mp4 2>&1|tee ~/tmp_ffmpeg/log2_fake_B.txt"
            % (img.shape[0], img.shape[1], video_path.replace("&", "\&"))
        )
        os.system(
            "ffmpeg -r 6 -f image2 -s %dx%d -i ~/tmp_ffmpeg/real_A/im%%04d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p %s/test_real_A.mp4 2>&1|tee ~/tmp_ffmpeg/log2_real_A.txt"
            % (img.shape[0], img.shape[1], video_path.replace("&", "\&"))
        )
