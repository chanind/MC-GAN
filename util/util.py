...
def save_image(image_numpy, image_path, rgb):
    ...
    if n_ch == 1:
        ...
        if not rgb:
            image_numpy = np.reshape(
                image_numpy,
                (image_numpy.shape[0], image_numpy.shape[1] * n_ch),
                order="F",
            )
        else:

            image_numpy_ = np.zeros(
                (image_numpy.shape[0], image_numpy.shape[1] * n_ch / 3, 3)
            )
            for i in range(n_ch / 3):
                image_numpy_[
                    :, i * image_numpy.shape[1] : (i + 1) * image_numpy.shape[1], :
